...
def example1():
    ...
    # f_out = vecfit.vector_fitting_rescale(f_test, s_test, n_pole=16, n_iter=10, has_const=False, has_linear=True)
    ...
    f_in.plot(s_test, x_scale='log', y_scale='db', color='b')
    f_out.plot(s_test, y_scale='db', color='r', linestyle='--')
    (f_out-f_in).plot(s_test, y_scale='db', color='k', linestyle='--')
