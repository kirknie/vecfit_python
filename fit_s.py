...
def fit_s(f, s, n_poles=10, n_iters=10, s_dc=0, s_inf=0, pole_wt=0):
    ...
    if s_dc and s_inf:
        ...
        if s_dc == 1:  # 1-s is bounded
            poles, residues, d, h = fit_z.fit_z(1-f, s, n_poles, n_iters, has_d=1, has_h=0, reflect_z=[0], pole_wt=pole_wt)
            d = 1-d
            residues = -residues
        ...
        if s_inf == 1:  # 1-s is bounded
            poles, residues, d, h = fit_z.fit_z(1-f, s, n_poles, n_iters, has_d=0, has_h=0, pole_wt=pole_wt)
            d = 1-d
            residues = -residues
